...
var Superlogin = function (_EventEmitter) {
	...
	_createClass(Superlogin, [{
		...
		value: function configure() {
			...
			if (_typeof(config.storage) === 'object' && config.storage.getItem && config.storage.setItem) {
			...
			var item = this.storage.getItem('superlogin.session');

			if (item.then) {
				item.then(function (result) {
					_this2._session = JSON.parse(result);
				});
			} else {
		...
		value: function getSession() {
			...
			if (!this._session) {
				...
				if (item.then) {
					item.then(function (result) {
						_this5._session = JSON.parse(result);
					});
				} else {
					this._session = JSON.parse(item);
				}
		...
		value: function refresh() {
			...
			return this._http.post(this._config.baseUrl + '/refresh', {}).then(function (res) {
				_this6._refreshInProgress = false;
				if (res.data.token && res.data.expires) {
					Object.assign(session, res.data);
					_this6.setSession(session);
					_this6._onRefresh(session);
				}
				return session;
			}).catch(function (err) {
				_this6._refreshInProgress = false;
		...
		value: function authenticate() {
			...
			return new Promise(function (resolve) {
				var session = _this7.getSession();
				if (session) {
					resolve(session);
				} else {
					_this7.on('login', function (newSession) {
						resolve(newSession);
					});
				}
			});
		...
		value: function register(registration) {
			...
			return this._http.post(this._config.baseUrl + '/register', registration, { skipRefresh: true }).then(function (res) {
				if (res.data.user_id && res.data.token) {
					res.data.serverTimeDiff = res.data.issued - Date.now();
					_this9.setSession(res.data);
					_this9._onLogin(res.data);
				}
				_this9._onRegister(registration);
				return res.data;
			}).catch(function (err) {
				throw parseError(err);
			});
			...
			return new Promise(function (resolve, reject) {
				...
				var _oauthInterval = setInterval(function () {
					if (_oauthWindow.closed) {
						...
						if (!_this14._oauthComplete) {
							_this14.authComplete = true;
				...
				window.superlogin.oauthSession = function (error, session, link) {
					if (!error && session) {
						...
						_this14.setSession(session);
						_this14._onLogin(session);
						return resolve(session);
					} else if (!error && link) {
						_this14._onLink(link);
						return resolve(capitalizeFirstLetter(link) + ' successfully linked.');
					}
					_this14._oauthComplete = true;
					return reject(error);
				};
			});
		}
